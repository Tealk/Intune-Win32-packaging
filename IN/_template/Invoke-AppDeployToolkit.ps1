...
if ($runningProcesses) {
    $regValue = Get-ItemProperty -Path $HKLMIntune -Name $appName -ErrorAction SilentlyContinue
    if ($null -ne $regValue) {
        [System.String]$DeployMode = 'Interactive'
    }
    else {
...
Write-ADTLogEntry -Message "Run Script in $DeployMode Mode"
...
[String]$appFile = 'DATEINAME'
[String]$appParameters = 'VERYSILENT' # Ohne Parameter Leerzeichen
[String]$CloseApps = 'PROZESSNAME'
[String]$uuid = ''
[String]$wingetID = 'false'

[String]$regkey = 'machine' # machine or user

if ($regkey -eq 'machine') {
    [String]$RegKeyPath = 'HKLM:\SOFTWARE\Microsoft\Intune'
} else {
    [String]$RegKeyPath = 'HKCU:\SOFTWARE\Microsoft\Intune'
}
...
function Install-ADTDeployment
{
    ...
    Show-ADTInstallationWelcome -CloseProcesses $CloseApps -AllowDefer -DeferTimes 3 -NoMinimizeWindows
    ...
    if (-not (Test-Path "$RegKeyPath")) {
        Write-ADTLogEntry -Message "Creating the registry path $($RegKeyPath)"
    ...
    If ($wingetID -ne 'false') {
        Start-ADTProcess -FilePath 'winget' -ArgumentList "install $wingetID --version $($adtSession.appVersion) --silent" -WindowStyle 'Hidden'
    }
    elseif ($appFile.EndsWith('.msi')) {
        Start-ADTMsiProcess -Action 'Install' -FilePath $appFile -ArgumentList "$appParameters"
    }
    else {
        Start-ADTProcess -FilePath "$appFile" -ArgumentList "$appParameters" -WindowStyle 'Hidden'
    }
    ...
    Set-ADTRegistryKey -Key "$RegKeyPath" -Name "$($adtSession.appName)" -Value "$($adtSession.appVersion)"
}
...
function Uninstall-ADTDeployment
{
    ...
    Write-ADTLogEntry -Message "Set removed registry key in ($adtSession.appName)"
    Set-RegistryKey -Key "$RegKeyPath" -Name "$appName" -Value "removed"
}
